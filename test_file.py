...
parser.add_argument('--cs_ratio', type=int, default=10, help='from {1, 5, 10, 20,30, 40, 50}')
...
Phi_data_Name = 'mask/mask_20.mat'
...
res_All = np.zeros([1, ImgNum], dtype=np.float32)
dense_All = np.zeros([1, ImgNum], dtype=np.float32)
...
print('\n')
output_data = "CS ratio is %d, Avg Proposed PSNR/SSIM for %s is %.2f/%.4f, Avg Rec Time is %.4f, Epoch is %d, mask ratio is %.4f \n" % (
